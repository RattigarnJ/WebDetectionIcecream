...
BASE_SAVE_PATH = os.path.join(os.path.expanduser("~"), "Documents", "IceCreamDetection" , "StandingContract")
...
def update_task_progress(task_id, progress, message, status="running"):
    """ ส่งความคืบหน้าไปยัง Flask endpoint """
    try:
        response = requests.post(
            "http://localhost:5000/update_progress",
            json={
                "task_id": task_id,
                "progress": progress,
                "message": message,
        ...
        if response.status_code == 200:
            print(f"อัปเดตความคืบหน้า: task_id={task_id}, progress={progress}%, message={message}, status={status}")
        else:
            print(f"ไม่สามารถอัปเดตความคืบหน้า: HTTP {response.status_code}")
    except Exception as e:
        print(f"ข้อผิดพลาดในการอัปเดตความคืบหน้า: {e}")
...
try:
    ...
    # Input contract type on input-box
    standicy_contract = driver.find_element(By.XPATH, "/html/body/div/div[2]/div/mat-dialog-container/div/div/app-dialog-multiselect/div/div[2]/angular2-multiselect/div/div[2]/div[3]/div[1]/input")
    standicy_contract.send_keys("ทรงยืน")
    time.sleep(2)

    # Click contract type
    standicy_contract = driver.find_element(By.XPATH, '/html/body/div/div[2]/div/mat-dialog-container/div/div/app-dialog-multiselect/div/div[2]/angular2-multiselect/div/div[2]/div[3]/div[3]/ul/li[2]/label')
    standicy_contract.click()
    time.sleep(2)
                                ...
                                try:
                                    ...
                                    if row_index == rows_count:
                                        ...
                                        if 5 <= row_index <= 10:
                                            ...
                                            #             if image_url:
                                            #                 # File name
                                            #                 filename = os.path.join(output_dir, f"{date}_{number}_ตู้ทรงยืน_{idx + 1}.jpg")
